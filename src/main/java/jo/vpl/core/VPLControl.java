...
//        System.out.println(mouseMode);
         ...
         * If there is already a select hub, then remove that one first
...
//                    unmarshal(ClassLoader.getSystemResourceAsStream("problem/expense.xml"));
        ...
        } catch (JAXBException | ClassNotFoundException | InstantiationException | IllegalAccessException | NoSuchMethodException | SecurityException | IllegalArgumentException | InvocationTargetException ex) {
            Logger.getLogger(VPLControl.class
                    .getName()).log(Level.SEVERE, null, ex);
        }
    }
